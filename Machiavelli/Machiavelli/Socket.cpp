...
ssize_t Socket::read(char *buf, ssize_t maxlen) const {
	ssize_t len = 0;
	// might come in parts
	while (ssize_t n = ::recv(sock, buf + len, static_cast<int>(maxlen - len), 0)) {
...
void Socket::write(const char *buf, size_t len) const {
	throw_if_min1(static_cast<int>(::send(sock, buf, len, 0)));
}
